...
interface SeatStore {
    ...
    loadSeatsForEvent: (eventId: string, date: string) => Promise<void>;
...
export const useSeatStore = create<SeatStore>((set, get) => ({
    ...
    loadSeatsForEvent: async (eventId: string, date: string) => {
        try {
            // Fetch category prices from the API
            const response = await fetch(`/api/events/${eventId}/pricing?date=${encodeURIComponent(date)}`);
