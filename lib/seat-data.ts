...
export function generateGridStadiumSeats(eventId?: string, categoryPrices?: Record<string, number>, bookedPercentage: number = 76): Seat[] {
    ...
    // Default pricing as fallback when no database pricing is available
    const defaultPrices = {
        'VVIP': 800,
        'VIP': 400,
        'Royal': 500,
    ...
    // Use dynamic prices if available, otherwise use defaults
    const finalPrices = categoryPrices && Object.keys(categoryPrices).length > 0 
        ? categoryPrices 
        : defaultPrices;
...
export function formatPrice(price: number): string {
    return `${price.toLocaleString()} ريال سعودي`;
}