...
/**
 * Fetch category prices for a specific event and date from the database
 */
export async function getEventCategoryPrices(eventId: string, date?: string): Promise<Record<string, number>> {
    try {
        if (!prisma) {
        ...
        const event = await prisma.event.findFirst({
            where: {
                id: eventId,
                ...(date && { date: { has: date } }) // Filter by the specific date if date is provided
            },
            include: {
                seats: {
        ...
        if (!event) {
            console.warn(`Event with ID ${eventId} and date ${date} not found.`);
            return {};
        }
